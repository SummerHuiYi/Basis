...
public class AssertMethod extends Assert{
	 ...
	 * @param exp_text
	 */
	public static void page_Element_Text_Equals(WebElement element,String exp_text){
		logger.info("检查页面元素文本内容包含预期内容");
		assertEquals(new BaseHandle().findElementGetText(element).contains(exp_text),true,"页面中没有匹配文字的元素");	
	}
	 ...
	 * @param exp_text
	 */
	public static void page_Element_Text_StartWith(WebElement element,String exp_text){
		logger.info("检查页面元素文本以预期内容开始");
		assertEquals(new BaseHandle().findElementGetText(element).startsWith(exp_text),true,"页面中没有匹配预期文字开始的元素");
	}
	 ...
	 * @param exp_text
	 */
	public static void page_Element_Text_EndWith(WebElement element,String exp_text){
		logger.info("检查页面元素文本以预期内容结束");
		assertEquals(new BaseHandle().findElementGetText(element).endsWith(exp_text),true,"页面中没有匹配预期文字结束的元素");
	}
	 ...
	 * @param isVisible
	 */
	public static void page_Element_Visible(WebElement element){
		boolean waitResult = false;
		try {
			waitResult = WaitMethod.visibilityOfElementLocated(element);
			logger.info("查找元素locator="+element);
		}catch (Exception e) {
			//异常就不抛出了
			waitResult =  false;
			logger.error("元素位置"+element+"不可见"+e.getMessage());	
		}
	 ...
	 * @param isVisible
	 */
	public static void page_Element_Not_Visible(WebElement element){
		boolean waitResult = false;
		try {
			waitResult =	WaitMethod.visibilityOfElementLocated(element);
			logger.info("查找元素locator="+element);
	 ...
	 * logger.info("检查下拉菜单选项与预期结果"); assertEquals(ListUtil.listCompare(new
	 * BaseHandle().selectGetOptions(by), exp),true,"下拉菜单选项与预期结果不一致"); }
	 */
