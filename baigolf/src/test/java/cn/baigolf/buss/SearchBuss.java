...
public class SearchBuss extends TestCaseBeforeAfterMethod{
	...
	private void search(String caseID,String webUrl,String testData,String expectedResult) throws InterruptedException {
		...
		SearchPageHandle searchPageHandle=new SearchPageHandle(DriverBase.ThreadDriver.get());
;		//搜索地区或高尔夫球场
		...
		//使用javascript写入今天
		((JavascriptExecutor)DriverBase.ThreadDriver.get()).executeScript("$('input[id=datetimepicker]').attr('readonly',false).val('"+ft.format(
				  date)+"')");
		//时间
